...
func GetSafeResourceName(name, namespace string, clusterGroupUpgrade *ranv1alpha1.ClusterGroupUpgrade, maxLength int, log *logr.Logger) string {
	...
	if !ok {
		...
		if log != nil {
			log.Info("Status.SafeResourceNames already annotation present",
				"safename", safeName,
				"namespace", namespace,
...
func NewSafeResourceName(name, namespace, suffix string, maxLength int, log *logr.Logger) (safename string) {
	...
	namespaceLength := utf8.RuneCountInString(namespace)
	maxGeneratedNameLength := maxLength - suffixLength - namespaceLength - finalDashLength
	var base string
	if utf8.RuneCountInString(name) > maxGeneratedNameLength {
